...
class GalleryFilters {
  ...
  sortCards(cards) {
    return [...cards].sort((a, b) => {
      switch (this.currentSort) {
        ...
        case "innovative": {
          const aInnovative =
            parseInt(a.dataset.submissionVotesInnovative, 10) || 0;
          const bInnovative =
            parseInt(b.dataset.submissionVotesInnovative, 10) || 0;
          return bInnovative - aInnovative;
        }
        case "inclusive": {
          const aInclusive =
            parseInt(a.dataset.submissionVotesInclusive, 10) || 0;
          const bInclusive =
  ...
  getTotalVotes(card) {
    ...
    const innovative =
      parseInt(card.dataset.submissionVotesInnovative, 10) || 0;
  ...
  updateResultsCount(count) {
    ...
    if (this.currentFilter !== "all") {
      const activeChip = document.querySelector(
        `[data-filter="${this.currentFilter}"]`,
      );
  ...
  updateURL() {
    ...
    history.pushState(
      { filter: this.currentFilter, sort: this.currentSort },
      "",
      newURL,
    );
  }
...
// Feedback Prompt Handler (shows prompts after voting, linked to voting)
class FeedbackPromptHandler {
  constructor() {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize feedback prompt handler (after voting system is ready)
  const feedbackPrompt = new FeedbackPromptHandler();

  // Wait for voting system to be ready
  const checkVotingSystem = setInterval(() => {
    if (window.votingSystem) {
      feedbackPrompt.init(window.votingSystem);
